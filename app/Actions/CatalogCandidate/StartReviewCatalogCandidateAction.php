...
class StartReviewCatalogCandidateAction
{
    public function execute(CatalogCandidate $candidate, User|int|null $reviewedBy = null): CatalogCandidate
    {
        return DB::transaction(function () use ($candidate, $reviewedBy): CatalogCandidate {
            ...
            if ($locked->status !== CatalogCandidateStatus::Discovered) {
                throw ValidationException::withMessages([
                    'status' => [
                        "This catalog candidate cannot be moved to under review from its current status ({$locked->status->value}).",
                    ],
                ]);
            }

            $locked->status = CatalogCandidateStatus::UnderReview;
    ...
    private function lock(CatalogCandidate $candidate): CatalogCandidate
    {
        ...
        if (! $locked instanceof CatalogCandidate) {
            throw ValidationException::withMessages([
                'status' => ['A deleted catalog candidate cannot be reviewed.'],
            ]);
        }
