...
class RejectCatalogCandidateAction
{
        ...
        return DB::transaction(function () use ($candidate, $reviewedBy): CatalogCandidate {
            ...
            if (! in_array($locked->status, $allowed, true)) {
                throw ValidationException::withMessages([
                    'status' => [
                        "This catalog candidate cannot be rejected from its current status ({$locked->status->value}).",
                    ],
                ]);
            }

            $locked->status = CatalogCandidateStatus::Rejected;
    ...
    private function lock(CatalogCandidate $candidate): CatalogCandidate
    {
        ...
        if (! $locked instanceof CatalogCandidate) {
            throw ValidationException::withMessages([
                'status' => ['A deleted catalog candidate cannot be rejected.'],
            ]);
        }
